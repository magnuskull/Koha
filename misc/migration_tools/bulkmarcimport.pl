...
GetOptions(
    ...
    'marcmodificationtemplate:s' => \$marc_modification_template, # @FIXME: Validation?
);
$biblios ||= !$authorities;
$insert  ||= !$update;
...
if ($authorities) {
    ...
    ($tagid, $subfieldid) = GetMarcFromKohaField("biblio.biblionumber", $framework);
    $tagid ||= "001";
}
...
RECORD: foreach my $record (@{$marc_records}) {
    ...
    ModifyRecordWithTemplate($marc_modification_template, $record) if ( $marc_modification_template );
        ...
        if ($authorities) {
            ...
            if ($record_id) {
                ...
                if ($record_has_added_items || $matched_record_id) {
                    ...
                    $record = GetMarcBiblio($record_id, 1);
                }
